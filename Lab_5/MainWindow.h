	...
	private: System::Void SubjectAdd_button_Click(System::Object^ sender, System::EventArgs^ e) {
		...
		try {
			...
			if (count > 0) {
				...
				// Получаем новый ID
				String^ getMaxIdQuery = "SELECT ISNULL(MAX(ID), 0) + 1 FROM " + tableName;
				SqlCommand^ getMaxIdCommand = gcnew SqlCommand(getMaxIdQuery, connection);
				int newId = (int)getMaxIdCommand->ExecuteScalar();

				// Вставляем новую строку
				String^ insertQuery = "INSERT INTO " + tableName + " (ID, SubjectName, Grade) VALUES (@ID, @SubjectName, @Grade)";
